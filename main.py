...
def run_for_fold(lr, fold, train_data, test_data, model, args, sampleround, epoch, device, experiment_id, pretrain, best_pretrain_metric=None):
    best_test_metric = best_pretrain_metric
    ...
    for e in range(epoch):
        ...
        with torch.no_grad():
            ...
            if best_test_metric is None or test_metrics[2][-1] > best_test_metric[2][-1]:
                torch.save(model, "checkpoints/{}/fold_bestmodel".format(experiment_id))
                print("Fold", fold, "Best test result updated in Epoch", e, "\n")
                log_f.write("Fold {} Best test result updated in Epoch {}\n\n".format(fold, e))
    ...
    if args.test:
        ...
        for fold in range(1, 11):
            ...
            if args.pretrain:
                ...
                pretrain_metrics = run_for_fold(args.lr, fold, train_data, test_data, model, args, 1, args.epochPRE, device, experiment_id_pretrain, True)

                print("Fold {} pretrain finished.".format(fold))

                fold_pretrain_emo_metric.append(pretrain_metrics[0])
                fold_pretrain_cau_metric.append(pretrain_metrics[1])
                fold_pretrain_pair_metric.append(pretrain_metrics[2])
            ...
            best_test_metric = run_for_fold(args.lr / 10, fold, train_data, test_data, model, args, args.sampleround, args.epochRL, device, experiment_id_train, False, [fold_pretrain_emo_metric[-1], fold_pretrain_cau_metric[-1], fold_pretrain_pair_metric[-1]] if len(fold_pretrain_pair_metric) > 0 else None)
